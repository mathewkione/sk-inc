window.addEventListener('load', () =>{
  ...
  const loginData = new Account('loginData')  //initiate Security data
  ...
  if (localStorage.getItem('securityData')) {
    ...
    securityCenter.storeDataInStorage([{
      uid: 316566968285,
      securityQuestion: {
        qs: 'which school did you study',
        ans: 'codecademy'
      },
      code: 354334,
      email: 'mkiprop131@gmail.com',
      phone: 708685803
    }
    ])
  ...
  if (localStorage.getItem('chiefSystemAdmin')) {
    ...
    console.log('our system can not find admin data!')    //chiefAdmin.storeDataInStorage({details:{chiefAdminPassword:"Sierra7z1&",chiefAdminUsername:"ceomathew",chiefAdminID:"ceodeeptech",chiefAdminTag:"cadmin"},appStats:{logins:[],passResets:[],messages:[],criticalLogs:[]}})
    let backup = {
      'details': {
        'chiefAdminPassword': 'Sierra7z1&',
        'chiefAdminUsername': 'ceomathew',
        'chiefAdminID': 'ceodeeptech',
        'chiefAdminTag': 'cadmin'
      },
      adminList: [
        {
          'name': 'Stephanie Lopez',
          'uid': 'slopez315784142393'
        },
        {
          'name': 'Jayson Preston',
          'uid': 2556290678522
        ...
        'messages': [
          {
            ...
            'uid': 'edinam0',
            'detail': 'Mehn this issue keeps on happening and i feel fucked up, fuck this shit and refund my godamn money, motherfuckers!',
            'handlerID': 6666665566,
            taskID: 677676677676,
            accomplished: false,
        ...
        'criticalLogs': [
          {
            ...
            'message': 'Attempted adminn login with wrong password!'
          },
          {
            'type': 'admin',
            'uid': [
  ...
  try {
    ...
    let adminListID = adminList.map(x=>{
      return x.uid
    })
    ...
    let chiefAdminTag = adminDetails().chiefAdminTag    //app stats
    ...
    let adminChecks = appData().adminChecks    //adminsitrative stats
    //ACCESS SAVED ACCOUNTS
    //localStorage.removeItem("loginData")
    let profilesSaved = loginData.getDataFromStorage()    //SESSION DATA
    if (sessionStorage.getItem('user')) {
      console.log('Login session is ready!')
    } else {
      console.warn('Login ready not ready, please wait..')
      let allSessionData = accessSessionData()      //let preferenceObject={visible:[{item:"#registration-page",displayType:"block"}],hidden:[{item:"#login-page" ,displayType:"none"},{item:"#session-field" ,displayType:"none"}]}//{item: ,displayType:}
      ...
      accountInit.storeDataInSession('user', [
        defaultSessionData,
        {
          selectors: preferenceObject
        }
      ])
    }
    ...
    function homePage() {
      ...
      addZeroToDate()      //homePage function ends here
      let homePost = document.querySelector('#general-notification')
      homePost.textContent = 'Make a mayotrics account and wait for our product launch!'
    ...
    function initiateTimeOutAlerts(field, info, lifeTime, interval) {
    ...
    function addElementToDisplayMemory(item, status) {
      ...
      if (item.length < 2) {
        console.log(`single-item ${ item }`)        //item of length greater than 1
    ...
    recallDisplay()    //GO TO LOGIN OR REGISTRATION FROM HOME PAGE
    ...
    = session
    const authLogin = regularID    //localStorage.removeItem("loginData")
    //OPERATIONS ON DATA
        ...
        if (pass === confirmPassword) {
          ...
          if (filter.length === 0) {
            ...
            accountLogs = [
              ...
              {
                uid: userID,
                log: 'Account created successfuly!'
              }
            ]
            const alertMessage = 'Account created successfuly!'
            initiateTimeOutAlerts(notificationField, alertMessage, 4, 1000)            //Initiate login tokens
            const sessionDataToUse = accessSessionData() //all session data
            ...
            for (let p = 0; p < sessionDataToUse.length; p++) {
              for (let key in sessionDataToUse[p]) {
                if (key === 'pageSession') {
                  sessionDataToUse[p][key] = {
                    regularID: userID,
                    generatedSessionID: randomSessionID
                  }
                  break
                }
              }
            ...
            if (matchWithUserObject.length > 0) {
              info = [
                ...info,
                'cant save this profile because it is already saved!'
              ]
            } else {
    ...
    registerBtn.addEventListener('click', createAccount)    //e.preventDefault()
    ...
    function checkUser(user, arg) {
      ...
      if (matchUser.length === 0) {
        info = [
          ...info,
          'the sever could not find your name, please retry or seek support!'
        ]
        return []
      } else {
        if (arg === 'match') {
    ...
    let loginBtn = document.querySelector('#login')    //start of login replacement code
    ...
    function login() {
      ...
      if (matchUser.length !== 0 && matchPassword.length !== 0) {
        ...
        logins = updatedSlogs        //save a profile
        let loginObject = {
          savedID: userID,
          savedName: user,
          savedPassword: pass
    ...
    loginBtn.addEventListener('click', login)    //login()//provide a button to display forms to rese password or to signup
    ...
    function updateSecurity() {
      const securityQuestionList = [
        ...
        'what is your faourite color',
        'what is your nickname'
      ]
      let secQuiz = securityQuestionList[1]
      let answerProvided = 'coding'      /*for(let i=0; i<securityQuestionList.length; i++){
...
mailAdressInput=email*/
    ...
    function securityManager() {
      ...
      if (matchName.length !== 0) {
        if (userSecurityData) {
          ...
          = userSecurityData
          const storedQs = securityQuestion.qs
          const storedAns = securityQuestion.ans          //server to create the code and send to user
          ...
          info = [
            ...info,
            'No security data for this account, please reach out to support quoting your user id : ' + userID
          ]
        }
      } else {
    ...
    const userLocation = locationData.filter(x=>{
      return x.uid === id
    }) [0] || [
    ] //to bes displayed in html if not null
    ...
    function getPreciseLocation() {
      ...
      const preciseLocationReasult = navigator.geolocation.getCurrentPosition(success, error, options)
    ...
    function addInputLocation() {
      ...
      let retrievedUserLocation = locationData.filter(x=>{
      ...
      if (retrievedUserLocation.length === 0) {
        const locationInput = {
          uid: regularID,
          inputLocation: enteredLocation,
          preciseLocation: preciseDeviceLocation || ''
        }
        locationData = [
          ...locationData,
          locationInput
    ...
    function addEmail() {
      let emailInput = {
        uid: regularID,
        email: 'slopez.mayotrics@gmail.com'
      }
      let retrievedEmail = emails.filter(x=>{
        return x.uid === regularID
      })
      if (retrievedEmail.length !== 0) {
        console.warn('the user has a saved email already!')
      } else {
        emails = [
          ...emails,
          emailInput
    ...
    function updateEmail() {
      const changeEmailTo = 'slopez.mayotricsdevelopers@gmail.com'
    ...
    function addALink() {
      ...
      if (checkAllLinks.length !== 0) {
        alert('links available')
        console.log(checkAllLinks)
      } else {
        const linkToAdd = 'www.mayotrics.com'
        accountLink = [
          ...accountLink,
          {
            uid: regularID,
    ...
    function setGoal() {
      id = regularID
      let userGoal = 'become full stack dev'
      let goalDetail = 'finish learning java, finish front ent with javascript'
      let userGoalCheck = myGoals.filter(x=>{
        return x.uid === id
      }) || [
      ]
      if (userGoalCheck.length !== 0) {
        info = [
          ...info,
          'the user has already set goals!'
        ]
      } else {
        info = [
          ...info,
        ...
        accountLogs = [
          ...
          {
            uid: id,
            log: `I set a goal : ${ userGoal } on ${ new Date().toISOString() },user id : ${ id }`
          }
        ]
      }
      function updateGoal() {
        let userGoal = 'raise funding by $200M'
        let goalDetail = 'make known our objectives to investors and project our possible milestone'
        ...
        for (let k = 0; k < myGoals.length; k++) {
          if (myGoals[k].uid === id) {
            ...
            accountLogs = [
              ...
              {
                uid: id,
                log: `changed goal from ${ goal },${ detail } to : ${ userGoal } : ${ goalDetail } on ${ new Date().toISOString() },user id : ${ id } `
              }
            ]
            break
          }
    ...
    function displayUserData() {
      ...
      if (userLinks.length !== 0) {
        ...
        const linksList = userLinks[1].links
        linksList.map(x=>{
          let eachLinkDiv = document.createElement('div')
          eachLinkDiv.textContent = x
    ...
    function setUpGoalDIV() {
      ...
      if (userGoal.length !== 0) {
        ...
        document.querySelector('#add-update').textContent = 'Update'
      } else {
        document.querySelector('#add-update').textContent = 'Add'
      }
    }
    ...
    function goToPreviousProfileItem() {
      ...
      const alertMessage = `current index :${ indexToShow }`
      initiateTimeOutAlerts(notificationField, alertMessage, 4, 1000)
      checkLocationFields()
    }
    ...
    document.querySelector('#skip-profile-field').addEventListener('click', goToNextProfileItem)    /*Edit location*/
...
document.querySelector("#switch-goal-ops").addEventListener('click',async ()=>{
let userGoal=myGoals.filter(x=>{
return x.uid===id
})||[]

if(userGoal.length!==0){
document.querySelector(".goals").value=userGoal[0].goals;
}
//
manageProfileField(userGoal.length!==0,[".edit-goal"],[".add-goal"],["#switch-goal-ops",".goals"])
})


document.querySelector("#switch-location-ops").addEventListener('click',async ()=>{
let userLoc=await locationData.filter(x=>{
return x.uid===id
})||[]

if(userLoc.length!==0){
document.querySelector("#new-location").value=userLoc[0].inputLocation;
}

let x=userLoc.length!==0
manageProfileField(x,[".edit-location"],[".add-location"],[".location-field","#switch-location-ops"])
})


document.querySelector("#switch-link-ops").addEventListener('click',async ()=>{
let userLinks=accountLink.filter(x=>{
return x.uid===id
})||[]
if(userLinks.length!==0){
document.querySelector("#add-link").value=userLinks[1].links;
}
let x=userLinks.length!==0
manageProfileField(x,[".edit-link"],[".add-link"],[".links","#switch-link-ops"])
})

document.querySelector("#switch-email-ops").addEventListener('click',async ()=>{
let userEmails=emails.filter(x=>{
return x.uid===id
})||[]
if(userEmails.length!==0){
document.querySelector("#new-email").value=emails[0].mail;
}
let x=userEmails.length!==0
manageProfileField(x,[".edit-email"],[".edit-email"],[".emails","#switch-email-ops"])
})

document.querySelector("#switch-home").addEventListener('click',async ()=>{
let userHome=myHome.filter(x=>{
return x.uid===id
})
let x=userHome.length!==0
manageProfileField(x,[".edit-home-items"],[".add-home-items"],[".home-items","#switch-home"])
})
*/
    //Event listeners for editting profile fields
    //add location
    //editlocation

    document.querySelector('#save-new-location').addEventListener('click', updateUserLocation)    /*
}
let userName=usernames.filter(x=>{
  return x.uid===id
})||[]
...
*/
      ...
      if (matchPassword !== 0) {
        if (currentPassword === matchPassword) {
          ...
          if (newPassword === matchPassword) {
            info = [
              ...info,
              'similar password is already stored!'
            ]
          } else {
            ...
            for (let i = 0; i < passwords.length; i++) {
              if (passwords[i].uid === userID) {
                ...
                accountLogs = [
                  ...
                  {
                    uid: passwords[i].uid,
                    log: `passsword successfully changed on ${ new Date().toLocaleString() }, user id : ${ passwords[i].uid }`
                  }
                ]
                break
              }
    ...
    function adminCenter() {
      ...
      function adminLogin() {
        ...
        if (tag === chiefAdminTag && userName === chiefAdminUsername && password === chiefAdminPassword) {
          ...
          accountInit.storeDataInSession('user', [
            {
              pageSession: {
                regularID: chiefAdminID,
                generatedSessionID: Math.floor(Math.random() * 1000000),
                tag: 'cadmin'
              }
            }
          ])
      ...
      function makeAdmin(adminUsername, adminID, adminName) {
        ...
        if (match.length !== 0) {
          info = [
            ...info,
            'the user is already registered as an admin!'
          ]
        } else {
      ...
      function chiefAdminPanel() {
        ...
        console.log(adminID)
        makeAdmin(adminUsername, adminID, adminName)        //removeAdmin(adminUsername,adminID,adminName)
      }
      chiefAdminPanel()
    ...
    function createMessage() {
      ...
      if (adminSelected) {
        ...
        info = [
          ...info,
          'No admin is available to handle the query!'
        ]
      }
    }
    adminList = adminList.filter(x=>{
    ...
    console.log('track taskCheck below1')    //createMessage()
    ...
    console.log(adminChecks)    //console.log(JSON.stringify(administrativeData()))
    ...
    saveAdministrativeData()    //console.log(usernames)
  } catch (e) {
    let prev = accountInitErr.getDataFromStorage() || [
    ]
    let err = ''
